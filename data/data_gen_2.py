...
if __name__ == "__main__":
    ...
    np.savetxt('sample/node_features.txt', node_features.numpy())
    np.savetxt('sample/edge_index.txt', edge_index.numpy().T, fmt='%d')  # 转置保存
    np.savetxt('sample/edge_weight.txt', edge_weight.numpy())
    
    # hetero_graph: (worker_features, task_features, hetero_edge_index, hetero_edge_type)
    worker_features, task_features, hetero_edge_index, hetero_edge_type = data['hetero_graph']
    np.savetxt('sample/worker_features.txt', worker_features.numpy())
    np.savetxt('sample/task_features.txt', task_features.numpy())
    np.savetxt('sample/hetero_edge_index.txt', hetero_edge_index.numpy().T, fmt='%d')
    np.savetxt('sample/hetero_edge_type.txt', hetero_edge_type.numpy(), fmt='%d')
    
    # correlation: (worker_sim_adj, task_sim_adj)
    worker_sim_adj, task_sim_adj = data['correlation']
    np.savetxt('sample/worker_sim_adj.txt', worker_sim_adj.numpy())
    np.savetxt('sample/task_sim_adj.txt', task_sim_adj.numpy())
    
    # metrics: {"q_matrix": q_matrix, "a_matrix": a_matrix, "demands": task_demands, "rewards": task_rewards}
    metrics = data['metrics']
    np.savetxt('sample/q_matrix.txt', metrics['q_matrix'].numpy())
    np.savetxt('sample/a_matrix.txt', metrics['a_matrix'].numpy())
    np.savetxt('sample/task_demands.txt', metrics['demands'].numpy())
    np.savetxt('sample/task_rewards.txt', metrics['rewards'].numpy())
    
    # worker_indices
    np.savetxt('sample/worker_indices.txt', data['worker_indices'].numpy(), fmt='%d')
    
    print("Data saved to sample/ directory as .txt files")